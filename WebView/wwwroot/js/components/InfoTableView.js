...
export class InfoTableView extends Component {
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (newState.tableCollection || newState.entityType) {
            return newState;
        }
        return null;
    }
    ...
    __getRows() {
        return this.state.tableCollection.slice(this.state.page * this.state.rowsPerPage, this.state.page * this.state.rowsPerPage + this.state.rowsPerPage).map((row) => {
            return (
                <TableRow
                    key={row.id}
                    tabIndex={-1}
                ...
                <Paper className={classes.root}>
                    <TableContainer>
                        <Table size="small" aria-label="purchases">
                            ...
                            <TableBody>
                                {this.state.tableCollection ? this.__getRows() : this.__getEmptyRow()}
                            </TableBody>
                        </Table>
                    </TableContainer>
                    <TablePagination
                        ...
                        component="div"
                        count={this.state.tableCollection?.length || 0}
